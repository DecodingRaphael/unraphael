...
if source_img is not None or default_image is not None:
    if st.sidebar.button('Detect Objects', key='detect_button'):
        ...
        res = model.predict(
            uploaded_image, conf=confidence, show_boxes=True, save=True, save_txt=True
        )
        boxes = res[0].boxes
        if len(boxes) == 0:
            ...
            if model_type == 'Detection':
                ...
                for idx, r in enumerate(res):
                    ...
                    for ci, kpts in enumerate(kptss):
                        ...
                        skeleton_legs = [
                            (11, 13),
                            (13, 15),  # Left Hip to Left Knee, Left Knee to Left Ankle
                            (12, 14),
                            (
                                14,
                                16,
