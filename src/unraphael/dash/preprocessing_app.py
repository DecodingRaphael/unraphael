...
def process_image(input_image, bilateral_strength, clahe_clip_limit, clahe_tiles,
                  ...
                  erode_size, alpha_matting, mask, post_process, background_color,mask_process= False):
...
def main():
    st.markdown('<div style="text-align: center;"><h1 style="color: orange;">Image preprocessing</h1></div>',
                unsafe_allow_html=True)
    st.markdown('<div style="text-align: center;"><h2 style="font-size: 26px;">For optimal background removal from '
                'an image</h2></div>', unsafe_allow_html=True)
    ...
    if uploaded_file is not None:        
        ...
        if uploaded_file is not None:
            ...
            processed_image_pil = process_image(image, bilateral_strength, clahe_clip_limit,
                                                                     clahe_tiles, sigma_sharpness, contrast,
                                                                     brightness, sharpening_kernel_size,
                                                                     saturation_factor, bg_threshold, fg_threshold,
                                                                     erode_size, alpha_matting, mask, post_process, background_color,mask_process= False)
            
            # Process mask with updated parameters
            processed_mask_pil = process_image(image, bilateral_strength, clahe_clip_limit,
                                                                     clahe_tiles, sigma_sharpness, contrast,
                                                                     brightness, sharpening_kernel_size,
                                                                     saturation_factor, bg_threshold, fg_threshold,
                                                                     erode_size, alpha_matting, mask, post_process, background_color,mask_process= True)
            ...
            # Apply mask to original image
            extracted_image_pil = apply_mask_and_convert_to_pil(np.array(image), np.array(processed_mask_pil))
